...
def get_init_bg(data):
    ...
    print(bg.shape, np.min(bg), np.max(bg))
    return bg
...
def train(network_architecture, minibatches, type='prodlda',learning_rate=0.001,
    ...
    l2_strength = np.ones([network_architecture["n_z"], network_architecture["n_hidden_gener_1"]]) / float(n_samples_tr)
    #l2_strength[0:4, :] = 0
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            cost, emb, bg = vae.partial_fit(batch_xs, l2_strength=l2_strength)
            ...
            l2_strength = 1.0 / weights_sq / float(n_samples_tr)
